...
use App\Livewire\Forms\LeadSourcesForm; // استخدام فورم مصادر العملاء
...
new #[Title('مصادر العملاء')] class extends Component
{
    ...
    #[Computed]
    public function leadSourcesList()
    {
        ...
        return LeadSource::query()
            ...
            ->when($this->created_from, fn($q) => $q->whereDate('lead_sources.created_at', '>=', $this->created_from))
            ->when($this->created_to, fn($q) => $q->whereDate('lead_sources.created_at', '<=', $this->created_to))
    ...
    public function toggleStatus($sourceId)
    {
        ...
        $source->save();
        $this->dispatch('notify', type: 'info', message: 'تم تغيير حالة مصدر العميل بنجاح.');
        unset($this->leadSourcesList);
    }
    ...
    public function deleteSource()
    {
        if ($this->sourceToDelete) {
            LeadSource::find($this->sourceToDelete)->delete();
            $this->showDeleteModal = false;
            $this->sourceToDelete = null;
            $this->dispatch('notify', type: 'warning', message: 'تم حذف مصدر العميل بنجاح.');
            unset($this->leadSourcesList);
        }
    }
    ...
    {
        if ($value) {
            $this->selectedIds = $this->leadSourcesList->pluck('id')->map(fn($id) => (string)$id)->toArray();
        } else {
            $this->selectedIds = [];
        }
    }
    ...
    {
        LeadSource::whereIn('id', $this->selectedIds)->delete();
        $this->selectedIds = [];
    ...
    // ===== إضافة مصدر جديد =====
    public function saveLeadSource(): void
    {
        $this->form->store();
        $this->dispatch('close-modal');
        $this->dispatch('notify', type: 'success', message: 'تم إضافة مصدر العميل بنجاح.');
        unset($this->leadSourcesList);
    }

    // ===== تحضير فورم التعديل =====
    public function editLeadSource(LeadSource $source): void
    {
        $this->form->setLeadSource($source);
        $this->dispatch('open-modal');
    }

    // ===== تحديث مصدر موجود =====
    public function updateLeadSource(): void
    {
        $this->form->update();
        $this->dispatch('close-modal');
    ...
    public function submitLeadSource(): void
    {
